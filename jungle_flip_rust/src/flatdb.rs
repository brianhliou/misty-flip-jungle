//! Memory-lean retrograde tablebase: a flat 2-bit value array with a combinatorial
//! PERFECT INDEX, so positions need no stored keys (unlike the HashMap builder in
//! `endgame.rs`). This is what scales toward ≤6 pieces.
//!
...
use crate::game::{apply, gen_moves, piece_count, result, EMPTY, NSQ};
...
pub struct FlatDB {
    levels: Vec<Bits>, // levels[k] for k pieces
}
...
impl FlatDB {
    ...
    /// Serialize to disk: `u32 num_levels`, then per level `u64 byte_len` + the packed bytes.
...
/// shared atomic working array (1 byte/position, packed to 2-bit per level at the end).
/// Returns `(db, per-level (n,w,l,d))`. Set `JF_FLAT_VERBOSE` for per-level timing.
pub fn build(max_pieces: usize) -> (FlatDB, Vec<(u64, u64, u64, u64)>) {
    ...
    for k in 1..=max_pieces {
        ...
        // working array: vec! fill (fast) then reinterpret as atomics (AtomicU8 is
        // repr(transparent) over u8, identical layout — sound).
        ...
        // pack to 2-bit (UNKNOWN ⇒ DRAW) + stats
        let mut label = Bits::new(n as u64, DRAW);
        let (mut w, mut l, mut d) = (0u64, 0u64, 0u64);
        for idx in 0..n {
            match wlabel(work[idx].load(Ordering::Relaxed)) {
                WIN => {
        ...
        if verbose {
            eprintln!("L{k}: n={n} {:.1}s rounds={round} W={w} L={l} D={d}", t_lvl.elapsed().as_secs_f64());
        }
